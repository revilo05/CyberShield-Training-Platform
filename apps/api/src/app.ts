...
export async function createApp(overrides: { config?: AppConfig; pool?: Pool } = {}): Promise<AppRuntime> {
  ...
  app.get(['/health', '/api/health'], async (_req, res, next) => { try { await repository.ping(); res.json({ status: 'ok', service: 'cybershield-api', persistence: 'postgresql', scoreVersion: 'HRS-2.0' }); } catch (error) { next(error); } });
  app.use('/api', createJobRouter(pool, config)); app.use('/api/auth', createEnterpriseAuthRouter(repository, config)); app.use('/api', createPlatformRouter(repository, config)); app.use('/api', createLegacyReportRouter(repository, config)); app.use('/api', createWebhookRouter(pool, repository, config)); app.use('/api', createExportRouter(pool, repository, config));
